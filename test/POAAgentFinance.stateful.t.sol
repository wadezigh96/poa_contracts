...
contract POAAgentFinanceHandler {
    ...
    function configure(uint256 cap, uint256 txLimit) external {
        ...
        poa.setCallPermission(agent, address(target), StatefulTarget.ping.selector, true);
        configuredCap = cap;
        configuredSpent = 0;
    }
    ...
    function execute(uint256 amount) external {
        ...
        (,, uint256 cap, uint256 spent, uint256 txLimit) = poa.agents(agent);
        ...
        (bool ok,) = agent.call(abi.encodeWithSelector(
            ...
            address(target), amount, abi.encodeWithSelector(StatefulTarget.ping.selector)
        ));
