...
contract POAAgentFinance {
    ...
    function revokeAgent(address agent) external onlyOwner { agents[agent].active = false; emit AgentRevoked(agent); }
    function setCallPermission(address agent, address target, bytes4 selector, bool allowed) external onlyOwner {
        ...
        allowedCalls[agent][target][selector] = allowed; emit CallPermissionSet(agent, target, selector, allowed);
    }
    ...
    function setTokenPolicy(address agent, address token, uint256 cap, uint256 txLimit) external onlyOwner {
        ...
        tokenPolicies[agent][token] = TokenPolicy(cap, 0, txLimit); emit TokenPolicySet(agent, token, cap, txLimit);
    }
    ...
    function withdrawNative(address payable to, uint256 amount) external onlyOwner nonReentrant {
        ...
        (bool ok, bytes memory reason) = to.call{value: amount}(""); if (!ok) revert CallFailed(reason); emit NativeWithdrawn(to, amount);
    }
    ...
    function execute(address target, uint256 value, bytes calldata data) external onlyActiveAgent nonReentrant returns (bytes memory result) {
        ...
        if (value > p.nativeTxLimit || p.nativeSpent + value > p.nativeCap || value > address(this).balance) revert CapExceeded();
        if (value > p.nativeTxLimit) revert TxLimitExceeded();
        p.nativeSpent += value;
        uint256 currentNonce = nonce++;
        (bool ok, bytes memory ret) = target.call{value: value}(data); if (!ok) revert CallFailed(ret);
    ...
    function agentRemainingNative(address agent) external view returns (uint256) {
        AgentPolicy memory p = agents[agent]; if (!p.active || p.expiresAt == 0 || block.timestamp >= p.expiresAt || p.nativeSpent >= p.nativeCap) return 0; return p.nativeCap - p.nativeSpent;
    }
